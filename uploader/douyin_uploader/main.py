...
class DouYinVideo(object):
    def __init__(self, title, file_path, tags, publish_date: datetime, account_file, thumbnail_path=None, productLink='', productTitle='',enableTimer=False):
    ...
    async def set_schedule_time_douyin(self, page, publish_date):
        ...
        await page.fill('.semi-input[placeholder="日期和时间"]', str(publish_date_hour))
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        # 2. 使用placeholder和class定位（用户指定的方式）
        title_input_by_placeholder = page.locator('.semi-input.semi-input-default[placeholder="填写作品标题，为作品获得更多流量"]')

        # 3. 备选的notranslate类定位
        titlecontainer = page.locator(".notranslate")

        if await title_input_by_placeholder.count():
            # 使用用户指定的方式定位并输入标题
            await title_input_by_placeholder.fill(self.title[:30])
        else:
        ...
        while True:
            ...
            try:
                ...
                await asyncio.sleep(2)
        #自助声明
        await self.auto_set_status(page, 1)
        #上传视频封面
        ...
        while True:
            ...
            try:
                ...
                await page.wait_for_url("https://creator.douyin.com/creator-micro/content/manage**",
                                        timeout=60000)  # 如果自动跳转到作品页面，则代表发布成功
        ...
        await context.close()
        await browser.close()
    async def auto_set_status(self, page: Page, status: int):
        # 自动设置状态
        ##DCPF > div > div.content-left-F3wKrk > div > div:nth-child(1) > div:nth-child(2) > section > div.controlWrapper-Kt_9Xm > div > div
        label_element = page.locator(".selectText-XSrMFZ")
        if await label_element.count():
        # 在选中的 label 元素下点击 checkbox
            await label_element.click()
            await asyncio.sleep(1)
            await page.locator('label.semi-radio').last.click()
            await page.locator('button.btn-xtdEbg').last.click()
            await asyncio.sleep(1)
    ...
    async def set_thumbnail(self, page: Page, thumbnail_path: str):
        ...
        if thumbnail_path =='' or thumbnail_path is None:
            # 选择智能推荐封面的第一个图片
            #await page.wait_for_selector(".content-upload-new", timeout=5000)
            first_cover = page.locator(".cover-Jg3T4p").nth(0)
            await first_cover.click()
             # 等待并点击确认按钮（根据提供的弹出框代码）
            try:
                # 等待确认对话框存在（不要求可见）
                #await page.wait_for_selector(".semi-modal-confirm", timeout=3000, state="attached")
                # 等待确认按钮可见并点击
                await asyncio.sleep(1)  # 这里延迟是为了方便眼睛直观的观看
                if await page.locator("span:has-text('暂不设置')").count():
                    await page.locator("span:has-text('暂不设置')").nth(0).click()

                confirm_button = page.locator("#tooltip-container").locator("button").nth(0)
                #await confirm_button.wait_for(timeout=3000, state="visible")
                #dialog-1 > div > div.dy-creator-content-modal-footer > div > button.semi-button.semi-button-primary.semi-button-light.secondary-s71anE > span
                ...
                # 如果没有确认窗口或点击失败，继续执行
                print(f"没有确认窗口或点击失败，继续执行，错误信息：{e}")

            # 点击完成按钮
            #await page.locator("button.semi-button.semi-button-primary.semi-button-light.secondary-zU1YLr").click()
        else:
